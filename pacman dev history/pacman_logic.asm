...
map_msg_fmt1 byte "%d %d %x %s", 0     ; 地图文件第一行格式
...
ghost_house_x equ 13 ; 幽灵出生点x
ghost_house_y equ 11 ; 幽灵出生点y
...
player_Pinky player_struct <11, 14, Dir_Up, Dir_Up, 1, Pinky, Ghost_State_Scatter, 0,1>
player_Inky player_struct <13, 14, Dir_Up, Dir_Up, 1, Inky, Ghost_State_Scatter, 0,1>
player_Clyde player_struct <15, 14, Dir_Up, Dir_Up, 1, Clyde, Ghost_State_Scatter, 0,1>
...
get_nxt_pos proc uses esi edi, x:dword, y:dword, dir:dword, isPacMan:dword
    ;根据dir计算下一个位置
    mov esi, x
    mov edi, y
    .if dir == Dir_Up
...
; 获取随机的NextDir
get_random_dir proc uses esi edi, nowX:dword, nowY:dword, nowDir:dword
    ...
    .while esi <= Dir_Right
        .if esi != reverseDir
            mov ebx, nowX
            mov ecx, nowY
            invoke get_nxt_pos, ebx, ecx, esi, 0
            .if eax != -1
                mov eax,dirListSize
...
player_move proc uses ecx, item:dword
    ...
    invoke get_nxt_pos, [ecx].player_struct.X, [ecx].player_struct.Y, [ecx].player_struct.Dir, isPacMan
    .if eax != -1
...
get_nxt_dir_by_target proc ,nowDir:dword, nowX:dword, nowY:dword, targetX:dword, targetY:dword
    ...
    .while esi <= Dir_Right
        .if esi != reverseDir
            mov eax, nowX
            mov ebx, nowY
            invoke get_nxt_pos, eax, ebx, esi, 0
            .if eax != -1
                ; 计算距离
...
nxt_dir_Blinky proc uses esi
    ...
    .elseif esi == Ghost_State_Scatter
        ...
        mov targetX, eax
        mov targetY, 0
    .elseif esi == Ghost_State_Eaten
        ; 目标goast house
        mov targetX, ghost_house_x
        mov targetY, ghost_house_y
    .elseif esi == Ghost_State_Frightened
        invoke get_random_dir, player_Blinky.X, player_Blinky.Y, player_Blinky.Dir
        mov player_Blinky.NextDir, eax
        mov player_Blinky.Dir, eax
        ret
    .endif
    invoke get_nxt_dir_by_target, player_Blinky.Dir, player_Blinky.X, player_Blinky.Y, targetX, targetY
    mov player_Blinky.NextDir, eax
    mov player_Blinky.Dir, eax
    ret
nxt_dir_Blinky endp
...
nxt_dir_Pinky proc uses esi
    ...
    .elseif esi == Ghost_State_Eaten
        ...
        mov targetY, ghost_house_y
    .elseif esi == Ghost_State_Frightened
        invoke get_random_dir, player_Pinky.X, player_Pinky.Y, player_Pinky.Dir
        mov player_Pinky.NextDir, eax
        mov player_Pinky.Dir, eax
        ret
    .endif
    invoke get_nxt_dir_by_target, player_Pinky.Dir, player_Pinky.X, player_Pinky.Y, targetX, targetY
    mov player_Pinky.NextDir, eax
    mov player_Pinky.Dir, eax
    ret
nxt_dir_Pinky endp
...
nxt_dir_Inky proc uses esi
    ...
    .elseif esi == Ghost_State_Eaten
        ...
        mov targetY, ghost_house_y
    .elseif esi == Ghost_State_Frightened
        invoke get_random_dir, player_Inky.X, player_Inky.Y, player_Inky.Dir
        mov player_Inky.NextDir, eax
        mov player_Inky.Dir, eax
        ret
    .endif
    invoke get_nxt_dir_by_target, player_Inky.Dir, player_Inky.X, player_Inky.Y, targetX, targetY
    mov player_Inky.NextDir, eax
    mov player_Inky.Dir, eax
    ret
nxt_dir_Inky endp
...
nxt_dir_Clyde proc uses esi
    ...
    .elseif esi == Ghost_State_Eaten
        ...
        mov targetY, ghost_house_y
    .elseif esi == Ghost_State_Frightened
        invoke get_random_dir, player_Clyde.X, player_Clyde.Y, player_Clyde.Dir
        mov player_Clyde.NextDir, eax
        mov player_Clyde.Dir, eax
        ret
    .endif
    invoke get_nxt_dir_by_target, player_Clyde.Dir, player_Clyde.X, player_Clyde.Y, targetX, targetY
    mov player_Clyde.NextDir, eax
    mov player_Clyde.Dir, eax
    ret
nxt_dir_Clyde endp
...
draw_player proc , item:dword,isOnCross:dword
    ...
    .endif
    invoke get_player_ptr, item
    invoke create_draw_item , [eax].player_struct.X, [eax].player_struct.Y, 3, item, [ebx].player_struct.Dir
    ret
draw_player endp
...
read_map proc uses esi edi
    ...
    mov file_ptr, eax
    invoke fscanf, file_ptr, offset map_msg_fmt1, offset map_rows, offset map_cols, offset wallColor, offset map_path
    ; 去除换行符
    ...
    ; 测试读取结果
    invoke printf, offset map_msg_fmt1, map_rows, map_cols, wallColor, offset map_path
